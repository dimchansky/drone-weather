...
export function RiskSummary({ risk, observedAt }: { risk: RiskSummaryT; observedAt?: Date }) {
  return (
    <section className={styles.wrap} style={{ borderColor: SEVERITY_VAR[risk.overall] }}>
      ...
      <ul className={styles.list}>
        {risk.components.map((c) => {
          const reason =
            c.key === 'freshness' && observedAt
              ? `Observed at ${fmtLocalTime(observedAt)} LT; ${c.reason}`
              : c.reason;
          return (
