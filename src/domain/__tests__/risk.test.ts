...
describe('assessRisk aggregation', () => {
  it('is GOOD overall for benign conditions', () => {
    ...
    const r = assessRisk({ metar, icingWorst: 'GOOD', icingReason: 'low', distanceKm: 5 });
  ...
  it('a NO-FLY component dominates', () => {
    ...
    const r = assessRisk({ metar, icingWorst: 'NOFLY', icingReason: 'freezing fog', distanceKm: 5 });
  ...
  it('reduced confidence bumps a GOOD up to CAUTION but never to NO-FLY', () => {
    const metar = m('LFPG 281200Z 27006KT CAVOK 20/05 Q1016');
    const farStale = assessRisk({ metar, icingWorst: 'GOOD', icingReason: 'low', distanceKm: 60 });
